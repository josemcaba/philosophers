...
/*   Updated: 2023/09/03 21:02:31 by jocaball         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	philos_mutex_init(t_philo *philo)
{
	...
	if (pthread_mutex_init(&(*philo).right_fork_mtx, NULL))
	...
	*philos = malloc(data->nbr_philos * sizeof(t_philo));
	...
	while (++i < data->nbr_philos)
	{
		...
		if (i > 0)
		{
			...
			(*philos)[i].left_fork_mtx = &(*philos)[i - 1].right_fork_mtx;
		}
	}
	(*philos)[0].left_fork = &(*philos)[data->nbr_philos - 1].right_fork;
	(*philos)[0].left_fork_mtx = &(*philos)[data->nbr_philos - 1].right_fork_mtx;
	return (EXIT_SUCCESS);
}
